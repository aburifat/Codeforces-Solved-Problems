...
int main()
{
    ...
    while(q--)
    {
        ll Lx=-EPS,Rx=EPS,Uy=EPS,Dy=-EPS;
        ll n;
        cin>>n;
        while(n--){
            ll x,y;
            ...
            ll a,b,c,d;
            cin>>a>>b>>c>>d;
            if((!a)&&Lx<x)Lx=max(Lx,x);
            if((!b)&&Uy>y)Uy=min(Uy,y);
            if((!c)&&Rx>x)Rx=min(Rx,x);
            if((!d)&&Dy<y)Dy=max(Dy,y);
        }
        if(Lx>Rx||Uy<Dy)cout<<0<<endl;
        else cout<<1<<' '<<Lx<<' '<<Dy<<endl;
    }
    return 0;
}
